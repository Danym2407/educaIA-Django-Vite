...
const BACKEND_ENROLL_URL = 'http://localhost:8000/api/enrollments/';
...
const CourseViewerPage = () => {
  ...
  // Cargar curso y seleccionar primera lección (SIEMPRE, no depende de autenticación)
  ...
  // Carga inicial SIEMPRE (no depende de autenticación)
  useEffect(() => {
    fetchAndSetCourseStructure();
  }, [courseId, fetchAndSetCourseStructure]);

  // Al cambiar de lección, cargar progreso de Django únicamente si autenticado
  useEffect(() => {
  ...
  // Función robusta para formatear segundos a mm:ss
  ...
  const handleEnroll = async () => {
    try {
      const response = await fetch(BACKEND_ENROLL_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('authToken')}`,
      ...
      if (response.ok) {
        toast({ title: "Inscripción exitosa", description: "Ya puedes acceder a los videos." });
        fetchAndSetCourseStructure(); // Refresca datos del curso
      } else {
        toast({ title: "Error al inscribirse", description: "Intenta nuevamente.", variant: "destructive" });
      }
    } catch (error) {
  ...
  if (!course) {
    return <div className="flex justify-center items-center h-screen bg-background"><p className="text-foreground">Cargando datos del curso...</p></div>;
  }
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-var(--header-height,4rem))] bg-background text-foreground">
      ...
      >
        {/* Bloquea SOLO el video y controles */}
        {!isAuthenticated ? (
          <div className="flex flex-col items-center justify-center h-64">
            <p className="mb-4 text-lg font-semibold">Debes iniciar sesión para ver el contenido del curso.</p>
            <Button onClick={() => navigate('/auth', { state: { from: location.pathname } })}>
              Iniciar sesión
            </Button>
          </div>
        ) : !currentLesson?.is_enrolled ? (
          <div className="flex flex-col items-center justify-center h-64">
            <p className="mb-4 text-lg font-semibold">Debes inscribirte para acceder a los videos.</p>
